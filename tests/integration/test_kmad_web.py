...
class TestApi(object):
    ...
    def test_sequence(self):
        ...
        """
        n = 1
        test_sequences = [self._get_random_sequence() for _ in range(n)]
    ...
    def _get_data(self, output_type, fasta_content):
        if output_type == 'align':
            ...
            prediction_methods = 'globplot psipred disopred d2p2 iupred ' \
                'predisorder'
            data = {'seq_data': fasta_content,
    ...
    def _get_random_sequence(self):
        response = requests.get(
            'http://uniprot.org/uniprot/?query=protein&random=yes&format=' \
            'fasta&fil=reviewed:yes')
        fasta = response.text
