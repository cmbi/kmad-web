...
@patch('kman_web.services.convert.open', mock_open(read_data=open('tests/unit/testdata/TEST.dat').read()), create=True)
...
def test_search_elm(mock_request, mock_urlopen):
    ...
    result = search_elm('TAU_HUMAN', seq, slims_classes)
    eq_(result, expected)
...
@patch('kman_web.services.convert.open', mock_open(read_data=open('tests/unit/testdata/test_elm_classes.tsv').read()), create=True)
def test_elm_db():
    from kman_web.services.convert import elm_db
    expected = {'TEST_ID': [0.003564849399, '"TEST_ACC"\t"TEST_ID"\t"description"\t"TESTREGEX"\t"0.003564849399"\t"39"\t"0"']}
